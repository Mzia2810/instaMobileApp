...
const SmsAuthenticationScreen = props => {
  ...
  const confirmEmailSignUpCode = smsCode => {
    authManager.confirmEmailSignUpCode(userInfo, smsCode).then(response => {
      setLoading(false)
      if (response.error) {
        Alert.alert(
          '',
  ...
  const confirmResetPassword = smsCode => {
    return props.navigation.replace('NewPassword', {
      smsCode,
      email: email.trim(),
    })
  }
  ...
  const onFinishCheckingCode = newCode => {
    setLoading(true)
    if (isConfirmSignUpCode) {
      confirmEmailSignUpCode(newCode)
      return
    }

    if (isConfirmResetPasswordCode) {
      confirmResetPassword(newCode)
      return
    }
    if (isSigningUp) {
